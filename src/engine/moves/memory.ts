...
export function registerMemoryMoves(
  ...
  registry.register({
    schema: {
      name: "list_pages",
      description:
        "List the names of all wiki pages in a scope (bible or journal).",
      parameters: {
        type: "object",
        properties: {
  ...
  registry.register({
    ...
    handler: async (args, _context) => {
      ...
      if (!page) {
        return {
          ok: false,
          log: `Page not found: ${args.name} in ${args.scope}`,
        };
      }
      return {
        ok: true,
  ...
  registry.register({
    schema: {
      name: "write_page",
      description:
        "Create or replace a wiki page with new content. Journal scope cannot have private content.",
      parameters: {
        type: "object",
        properties: {
          scope: { type: "string", enum: ["bible", "journal"] },
          ...
          public: {
            type: "string",
            description: "Publicly known facts or events.",
          },
          private: {
            type: "string",
            description: "GM-only secrets or hidden motivations (Bible only).",
          },
        },
  ...
  registry.register({
    schema: {
      name: "append_page",
      description:
        "Append HTML content to the public section of an existing wiki page.",
      parameters: {
        type: "object",
        properties: {
          scope: { type: "string", enum: ["bible", "journal"] },
