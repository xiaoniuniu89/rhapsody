...
export default class JournalBrowserApp extends Base {
  ...
  async _preparePartContext(partId: string, context: JournalBrowserContext): Promise<any> {
    switch (partId) {
      case 'header':
        return {
          ...context,
  ...
  private getAllFolders(): JournalFolderData[] {
    ...
    return game.folders
      ...
      .map(folder => {
        ...
        const journalCount = game.journal.filter(j => j.folder?.id === folder.id).length;
  ...
  static async setFolderFilter(this: JournalBrowserApp, event: Event, target: HTMLElement) {
  ...
  static async clearFilter(this: JournalBrowserApp, event: Event, target: HTMLElement) {
  ...
  static async refreshJournals(this: JournalBrowserApp, event: Event, target: HTMLElement) {
  ...
  static async openJournal(this: JournalBrowserApp, event: Event, target: HTMLElement) {
