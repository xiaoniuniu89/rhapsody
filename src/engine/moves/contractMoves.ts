...
export function registerContractMoves(registry: MoveRegistry) {
  registry.register({
    schema: {
      name: "reveal_clue",
      description: "Reveal a new clue or piece of information to the player that was marked as 'on offer' in the scene contract.",
      parameters: {
        type: "object",
        properties: {
          clue: { type: "string", description: "The text or ID of the clue to reveal." }
        },
        required: ["clue"]
      }
    },
    handler: async (args, context) => {
      ...
      const match = contract.active.onOffer.find(c => c.text === clue || c.id === clue);
      
      if (!match) {
        return { ok: false, log: `Clue "${clue}" is not available in the current scene contract.` };
      }
  ...
  registry.register({
    schema: {
      ...
      description: "Introduce a specific complication or threat that was listed in the scene contract.",
      parameters: {
        type: "object",
        properties: {
          threat: { type: "string", description: "The text or ID of the threat to introduce." }
        },
        required: ["threat"]
      }
    },
    handler: async (args, context) => {
      ...
      const match = contract.active.complications.find(c => c.text === threat || c.id === threat);

      if (!match) {
        return { ok: false, log: `Threat "${threat}" is not in the scene contract complications.` };
      }

      await contract.recordProgress({
        complicationsTriggered: [...contract.active.progress.complicationsTriggered, match.id]
      });
  ...
  registry.register({
    schema: {
      ...
      parameters: {
        type: "object",
        properties: {
          choice: { type: "string", description: "The hard choice being offered." }
        },
        required: ["choice"]
      }
    },
    handler: async (args, context) => {
      ...
      if (contract.active) {
        await contract.recordProgress({
          freeform: [...contract.active.progress.freeform, {
            type: "hard_choice",
            text: args.choice,
            at: Date.now()
          }]
  ...
  registry.register({
    schema: {
      ...
      parameters: {
        type: "object",
        properties: {
          location: { type: "string", description: "The name of the location to cut to." },
          description: { type: "string", description: "Brief description of the cut." }
        },
    ...
    handler: async (args, context) => {
      ...
      if (contract.active && contract.active.exits.length > 0) {
        const isValid = contract.active.exits.some(e => e.toLowerCase() === args.location.toLowerCase());
        if (!isValid) {
          return { ok: false, log: `Location "${args.location}" is not a valid exit for this scene.` };
        }
      }
