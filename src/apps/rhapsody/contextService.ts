...
export class ContextService {
  ...
  async buildContextMessages(messages: Message[], sceneHistory: Scene[], systemInfo: string, worldName: string, currentSceneName: string): Promise<any[]> {
  ...
  shouldCompressContext(messages: Message[], sceneHistory: Scene[]): boolean {
    return this.getCurrentContextSize(messages, sceneHistory) > this.maxContextTokens;
  }

  getCurrentContextSize(messages: Message[], sceneHistory: Scene[]): number {
    let tokens = 0;
    tokens += this.estimateTokens(this.contextSummary || '');
    tokens += this.estimateTokens(sceneHistory[sceneHistory.length - 1]?.summary || '');
  ...
  async compressOlderMessages(messages: Message[]): Promise<{ updatedMessages: Message[], summary: string }> {
    const recentMessages = this.getRecentMessages(messages);
    ...
    if (recentMessages.length > 5) {
      ...
      const markerIndex = updatedMessages.findIndex(m => m.id === toKeep[0].id);
